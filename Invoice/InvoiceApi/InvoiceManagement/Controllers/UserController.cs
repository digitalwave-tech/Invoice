...
namespace InvoiceManagement.Controllers
{
    ...
    public class UserController : ControllerBase
    {
        ...
        public async Task<IActionResult> CreateInvoice([FromBody] InvoiceRequest invoices)
        {
            try
            {
                var identity = User.Identity as ClaimsIdentity;
                ...
                var user_ = await invoice_.BindInvoice(invoices);
                if (user_ == "500")
                {
        ...
        public async Task<IActionResult> GetInvoiceItemDetails([FromBody] List<int?> invoice_No)
        {
            try
            {
                ...
                //SgstRate = pDFTemplate[0].companyDetails.customerdetails[0].itemDetails[0].SgstRate;
                //CgstRate = pDFTemplate[0].companyDetails.customerdetails[0].itemDetails[0].CgstRate;

                //gstamt = Math.Truncate(100 * (subtotal * SgstRate / 100)/100??0);
                ...
                //htmlContent += "<td colspan='4' style = 'padding: 20px;width:100%; float:right; border-top: 1px solid #ddd;' >" + SgstRate + "%" + "</td>";
                //htmlContent += "</tr>";
                ...
                var response = new { success = true, msg = pDFTemplate };
                return Ok(response);
            }
            catch (Exception ex)
            {
